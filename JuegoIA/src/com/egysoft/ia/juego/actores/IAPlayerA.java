 ...
 * @author Edgardo
 */
public class IAPlayerA extends Player
{
    ...
    public IAPlayerA(String assetName, TextureAtlas atlas, Sound sound)
    {
    	setDebug(true);
    ...
    public class IdleState implements State
    {
		...
		{	
			...
			Array<Cube> fichas = actual.t.get(Cube.class);
			Array<Lair> lairs  = actual.t.get(Lair.class);
			for(int i=0;i<fichas.size;i++) //a la antiguita no má
			{
				final Cube fichaA   = fichas.get(i);
				for(int j=0;j<lairs.size; j++)
				{
					final Lair lair  = lairs.get(j);
					Array<Estado> ruta1 = algoritmo.buscar(lair.getCeldaActual(),fichaA.getCeldaActual());				
    ...
    public class LeftState implements State
    {
		...
		{		
			...
			if(!celda.Disponible(IPushable.class))
			{
				...
				if(cubo.push(IAPlayerA.this, -1, 0)) return;				
			}
    ...
    public class DownState implements State
    {
		...
		{		
			...
			if(!celda.Disponible(IPushable.class))
			{
				//ohh!!! un cubo!!! que hacemos con el?				
				IPushable cubo = (IPushable)celda.getPiezaActual();
				if(cubo.push(IAPlayerA.this, 0, -1))return;
			}
			if(celda.Disponible())
			{
				addAction(Actions.sequence(
			...
			if(operaciones != null && operaciones.size > 1)
			{
				...
				do
				{
					if(estado != null && Operacion.Inicial != estado.operacion)
					{
						...
						if(!sigA.celda.Disponible(Cube.class))
						{
							evitar = sigA.celda;
							setState(idleState);
							return true;														
	...
	private class EvitarCelda implements BusquedaAStar.H
	{
		...
		public int h(Celda actual) 
		{
			...
			if(actual != destino && !actual.Disponible(Lair.class))
			{
				h = h * 1000;
			}
